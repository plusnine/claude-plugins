...
After Step 3 (investigation validated by the user), append a new entry per `../../shared/references/code-map-format.md` Write policy. Concept derivation: task flow — see Column specification in `code-map-format.md`. Skip if not in a git repo or agent returned no Starting Points.
